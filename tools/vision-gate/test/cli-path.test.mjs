...
describe('Vision Gate CLI paths', () => {
  it('resolves paths relative to the pnpm caller directory', () => {
    ...
    try {
      const resolved = resolveCliPath('work/vision-gate/manifest.json')
      assert.equal(isAbsolute(resolved), true)
      assert.match(resolved.replaceAll('\\', '/'), /ai-bead-pattern\/work\/vision-gate\/manifest\.json$/)
    } finally {
