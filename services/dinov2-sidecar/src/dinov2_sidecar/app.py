...
from .contracts import MODEL_DESCRIPTOR, PROVIDER_ID, SCHEMA_VERSION, PairRequest
...
def _regional_comparison(view) -> dict[str, Any]:
    return {
        "view": view.view,
        "identitySimilarity": view.identity_similarity,
        "patchCorrespondence": view.patch_correspondence,
        "criticalPatchRetention": view.critical_patch_retention,
        "regionalCoverage": view.regional_coverage,
        "confidence": view.confidence,
    }


def create_app(engine: DinoV2PairEngine | None = None) -> FastAPI:
    runtime = engine or DinoV2PairEngine()
    app = FastAPI(title="AI Bead DINOv2 Pair Sidecar", version="0.1.0")
    ...
    async def analyze(
        ...
        for view in score.views:
            for metric_name, value in (
                ...
                names.append(f"{view.view}.{metric_name}")
                values.append(value)
        return {
            ...
            "preferenceFeatures": {
                ...
                "regionalComparisons": [
                    _regional_comparison(view) for view in score.views
                ],
            },
            "warnings": [
