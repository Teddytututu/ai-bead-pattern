...
function benchmarkSubjectMask(size: number): Float32Array {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      if (ellipse <= 1 && (x + y) % 29 !== 0) values[y * size + x] = 1
    }
  }
  return values
}
...
it('keeps an eighteen-candidate subject-shape load inside the CI budget', async () => {
  ...
  assert.equal(result.status, 'success')
  assert.equal(result.evaluation.rankedCandidateIds.length, 18)
