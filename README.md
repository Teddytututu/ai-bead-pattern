...
当前阶段已完成 v0.3.1 画布规划与特征预算。`pattern-core` 将视觉模型提供的主体 mask 转为目标豆格占用区域，保留连通块、孔洞和硬关键点；自动画布进一步比较主体占用、轮廓承载、构图、五官格数和制作成本。`ai-gateway` 通过 rembg 调用 `birefnet-general-lite`，将主体掩码、边界重要度、建议裁剪和模型版本转换为 `ImageAnalysis`。A0/A1 继续提供全画面对照，结构版负责主体形状、画布规划、颜色设计和网格整理。
